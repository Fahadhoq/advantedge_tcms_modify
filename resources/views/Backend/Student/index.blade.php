                                    ...
                                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                                ...
                                                <thead class="thead-default">
                                                    <tr>
                                                        ...
                                                        <th style=text-align:center>Student Create at</th>
                                                        <th style=text-align:center>Student is Active</th>
                                                ...
                                                @foreach($users as $user)
                                                    <tr>
                                                        ...
                                                        <td style=text-align:center>
                                                        <label><a href="{{ route('student.view' , @$user->id) }}" class="hover" id="{{$user->id}}">{{@$user->name}}</a></label>    
                                                        </td>
                                                        ...
                                                        <a href="{{ route('student.view' , $user->id) }}" class="btn btn-info btn-sm" title="View student"><i class="fa fa-eye"></i></a>
                                                       
                                                        <a href="{{ route('student.edit' , $user->id) }}" class="btn btn-warning btn-sm" title="Edit student"><i class="fa fa-edit"></i></a>
                                                            ...
                                                            @foreach($User_Infos as $User_Info)
                                                              @if($user->id == $User_Info->user_id)
                                                               @php $verified_by = App\Models\User::select('id','name')->where('id' , $User_Info->verified_by)->first() @endphp
                                                                  @if($verified_by != null)
                                                                  <a href="{{ route('student.view' , $verified_by->id) }}" class="btn btn-outline-info" title="View verifyed by student info">{{$verified_by->name}}</a>
                                                                       
                                                                  @else
                                                                    @foreach($de_users as $de_user)
                                                                     @if($de_user->id == $User_Info->verified_by)
                                                                     <a href="{{ route('student.view' , $de_user->id) }}" class="btn btn-outline-info" title="View verifyed by student info">{{$de_user->name}}</a>
                                                                     @endif    
                                                                    @endforeach
                                                                  @endif
                                                              @endif
                                                    ...
                                                    <a class="status-btn btn btn-success btn-sm" data-id="{{$user->id}}" title="verify student"><i class="fa fa-check" aria-hidden="true"></i></a>
                                                    @endif
                                                        </td>
                                                        <!-- change status end --> 
...
// filter created_at date
           ...
           $('#filter').click(function(){  
                ...
                {  
                     $.ajax({  
                          url: '/student-create-at-date-filter',  
                          type: 'post',  
                          data:{
                               from_date:from_date,
                               to_date:to_date,
...
$('#datatable-buttons').delegate('.status-btn', 'click', function(){
   ...
   // var table = $('#datatable-buttons').DataTable();
    ...
    $.confirm({
                title: 'Confirm!!!',
                content: 'Are you sure To verify this student?',
                buttons: {
                    confirm: function () {
                        $.ajaxSetup({
                    headers: {
